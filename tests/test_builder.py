...
from mytardis_rocrate_builder.src.rocrate_builder import ROBuilder, MT_METADATA_SCHEMATYPE
from mytardis_rocrate_builder.src.rocrate_dataclasses.rocrate_dataclasses import(
    MT_METADATA_TYPE,
    Organisation,
    Person,
    BaseObject,
...
def test_add_metadata(builder, test_mytardis_metadata, test_rocrate_metadata):
    crate_metadata = builder._add_metadata_to_crate(test_mytardis_metadata, metadata_id=test_mytardis_metadata.name, parent_id= "./")
...
def test_add_context_entity(builder: ROBuilder, test_context_object, test_rocrate_context_entity):
    assert builder.add_context_object(test_context_object).properties() == test_rocrate_context_entity.properties()
...
def test_add_acl(builder: ROBuilder,test_org_ACL:ACL, test_crate_ACL:ROContextEntity):
    assert test_crate_ACL == builder.add_acl(test_org_ACL)

def test_adda_additional_properites(builder: ROBuilder, test_extra_properties, test_context_object, test_properties_with_Context_obj, test_rocrate_context_entity):
    properties={}
    assert builder._add_additional_properties(properties, test_extra_properties) == test_extra_properties
    properties={}
    test_extra_properties["Context_obj"] = "#"+test_context_object.id
    assert builder._add_additional_properties(properties, test_properties_with_Context_obj) == test_extra_properties
    assert builder.crate.dereference("#"+test_context_object.id) == test_rocrate_context_entity
...
def test_ro_crate_project(
    ...
    return ROContextEntity(
        ...
        "metadata":["_".join(["Project_"+metadata.name]) for metadata in test_metadata_list.values()],
...
def test_ro_crate_experiment(
    ...
    return ROContextEntity(
        ...
        "datePublished":ro_date,
        'hasDigitalDocumentPermission':["#"+acl.id for acl in test_acl_list],
        "metadata":["_".join(["experiment_"+metadata.name]) for metadata in test_metadata_list.values()],
            } | test_extra_properties,
        )
...
def test_ro_crate_dataset(
    ...
    return RODataset(
        ...
        'hasDigitalDocumentPermission':["#"+acl.id for acl in test_acl_list],
        "metadata":["_".join([test_directory.as_posix()+metadata.name]) for metadata in test_metadata_list.values()],
            } | test_extra_properties,
        )
...
def test_add_dataset(builder, test_dataset, test_ro_crate_dataset):
    assert builder.add_dataset(test_dataset).properties() == test_ro_crate_dataset.properties()
